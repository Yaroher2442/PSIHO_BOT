...
class Answer:
    ...
    def validate_user(self):
        try:
            ...
            print(e)
            try:
                self.user_obj = TgClient.create(tg_id=self.message.from_user.id, status=None)
    ...
    def get_txt_answr_obj(self):
        try:
            if self.button_pressed:
                self.text_answr_obj = TextAnswers.get(TextAnswers.answer == self.button_pressed.text)
            else:
                try:
                    self.text_answr_obj = TextAnswers.get(TextAnswers.answer == self.message.text)
                except:
                    for answer in TextAnswers.select():
                        if answer.use_same_texts and fuzz.WRatio(answer, self.message.text) > 80:
